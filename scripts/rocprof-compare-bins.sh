#!/usr/bin/env bash
# rocprof-compare-bins.sh — Run rocprof on two already-linked executables; write perf_compare.json
#
# Usage:
#   ./scripts/rocprof-compare-bins.sh path/to/before_bin path/to/after_bin [output_dir]
#
# Same aggregation logic as run-rocprof.sh (5 runs by default, PERF_RUNS=N).

set -euo pipefail

BEFORE_BIN="${1:?Usage: $0 before_bin after_bin [output_dir]}"
AFTER_BIN="${2:?Usage: $0 before_bin after_bin [output_dir]}"
OUTDIR="${3:-web}"
...
run_rocprof "$BEFORE_BIN" "before" "$TMPDIR/prof_before"
run_rocprof "$AFTER_BIN" "after" "$TMPDIR/prof_after"
